...
func (s *server) SayHello(ctx context.Context, req *pb_helloworld.HelloRequest) (*pb_helloworld.HelloReply, error) {
	...
	conn, err := grpc.Dial(addr, grpc.WithInsecure()) //todo: add tracing here
	if err != nil {
	...
	// send message
	fmt.Printf("Vide Fragment length: %v", len(videoFragment))
...
func main() {
	...
	decoderAddr := flag.String("addr", "http://decoder.default.192.168.1.240.sslip.io ", "Decoder address")
	...
	fmt.Printf("read video fragment, size: %v\n", len(videoFragment))
